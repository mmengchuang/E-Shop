            ...
            if (err == null) {
                if (result.length > 0) {//库存充足
                    ...
                    connect.query(sqls[1], [u_id, o_time], function (err, result) {//向订单表插入数据
                        if (err == null) {
                            connect.query(sqls[2], [result.insertId, g_id, g_count], function (err, result) {//向订单详情信息表插入数据
                                if (err == null) {//下单成功
                                    connect.query(sqls[3], [(g_num - g_count), g_id], function (err, result) {//更新商品库存
                                        if (err == null) {
...
exports.addCar = function (req, res) {
    ...
    var g_id = req.body.g_id;//商品id
    var g_count = req.body.g_count;//商品购买数量
    var sqls = ['insert into cart(u_id) values(?)',//向购物表插入数据
        'insert into cartinfo(c_id,g_id,g_count) values(?,?,?)',//购物车详情信息表插入数据
    ];
    connect.beginTransaction(function (err) {
        if (err) {
            throw err;
        }
        connect.query(sqls[0], [u_id], function (err, result) {//向购物车表插入数据
            if (err) {
                return connect.rollback(function () {
                    res.writeHead(200, {'Content-Type': 'application/json'});
                    var data = {error_code: '4015', msg: err};
                    res.end(JSON.stringify(data));
                });
            }
            connect.query(sqls[1], [result.insertId, g_id, g_count], function (err, result) {//向购物车详情表中添加数据
                if (err) {
                    return connect.rollback(function () {
                        res.writeHead(200, {'Content-Type': 'application/json'});
                        var data = {error_code: '4013', msg: err};
...
exports.registerUser = function (req, res) {
    ...
    connect.beginTransaction(function (err) {
        ...
        connect.query(sql[0], [uname, upwd], function (err, result) {
            ...
            connect.query(sql[1], [result.insertId], function (err, result) {
                if (err) {
                    return connect.rollback(function () {//数据回滚操作
                        //throw err;
                        res.writeHead(200, {'Content-Type': 'application/json'});
